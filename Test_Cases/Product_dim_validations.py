...
logger = Logs.Log_Gen(f'C:\\NSR_Python_Projects\\Python_Project_Version2\\ETL_Testing_Framework(Config Driven Approach)\\Logs\\product_dim_{dt}.log')

with open(r'C:\NSR_Python_Projects\Python_Project_Version2\ETL_Testing_Framework(Config Driven Approach)\Config\SQL_Queries_config\Product_dim.json','r') as Product_data_SQL_fle:
...
def Column_mapping_Validation(source_db_conn, target_db_conn, Product_SQL_Queries, logger):
    ...
    if set(source_columns) == set(target_columns):
        ...
        if source_result.shape[0] == target_result.shape[0]:
            # in pandas dataframe shape[0] gives no of rows,shape[1] give no of columns
            ...
            if comparison_result.all().all():
                #The first all() confirms that all values in each column are True,
                # and the second all() confirms that this condition holds for all columns.
                ...
                mismatched_indices = comparison_result[comparison_result.all(axis=1)==False].index
                #This line is used to identify the indices of rows in the comparison_result DataFrame that
                # contain at least one mismatch (i.e., at least one False value),comparison_result.all(axis=1) == False: This explicitly checks for rows where  all values are False
...
dt = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
output_file_path = f"C:\\NSR_Python_Projects\\Python_Project_Version2\\ETL_Testing_Framework(Config Driven Approach)\\Output_Result\\Product_dim_Result_{dt}.xlsx"
