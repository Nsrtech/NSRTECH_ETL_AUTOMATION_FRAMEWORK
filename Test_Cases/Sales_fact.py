...
logger=Logs.Log_Gen(f'C:\\NSR_Python_Projects\\Python_Project_Version2\\ETL_Testing_Framework(Config Driven Approach)\\Logs\\Sales_fact_{dt}.log')

with open(r'C:\NSR_Python_Projects\Python_Project_Version2\ETL_Testing_Framework(Config Driven Approach)\Config\SQL_Queries_config\Sales_fact.json','r') as Sales_fact_data_SQL_fle:
    Sales_Fact_SQL_Queries = json.load(Sales_fact_data_SQL_fle)
def Source_Target_Count_check(source_db_conn,target_db_conn,Sales_Fact_SQL_Queries,logger):
    source_result = pd.read_sql(Sales_Fact_SQL_Queries['count_comparison']['source_query'],source_db_conn)
    target_result = pd.read_sql(Sales_Fact_SQL_Queries['count_comparison']['target_query'],target_db_conn)
...
def Column_mapping_Validation(source_db_conn,target_db_conn,Product_validations,logger):
    source_result = pd.read_sql(Sales_Fact_SQL_Queries["column_mapping"]["source_query"],source_db_conn)
    target_result = pd.read_sql(Sales_Fact_SQL_Queries["column_mapping"]["target_query"],target_db_conn)
...
counts = Source_Target_Count_check(source_db_conn,target_db_conn,Sales_Fact_SQL_Queries,logger)
null_chk = Null_Checks(target_db_conn,Sales_Fact_SQL_Queries,logger)
duplicates = Duplicate_Records_chk(target_db_conn,Sales_Fact_SQL_Queries,logger)
columns_mapp = Column_mapping_Validation(source_db_conn,target_db_conn,Sales_Fact_SQL_Queries,logger)
...
dt = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
output_file_path = f"C:\\NSR_Python_Projects\\Python_Project_Version2\\ETL_Testing_Framework(Config Driven Approach)\\Output_Result\\Sales_fact_Result_{dt}.xlsx"
